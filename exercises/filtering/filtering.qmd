...
Julian Reese is the top result with a trueshooting percentage of 60.76%. 
...
```{r}
matches_2024 |>
  filter(team == 'Maryland Terrapins, Terps', date < '2024-09-01') |>
  group_by(team) |>
  summarise(
    total_attempts = sum(sh_att),
    total_on_goal = sum(so_g),
...
Maryland has seen significant increases in each category from the 2024 to 2025 season. Maryland has 40 more attempts in this time frame, more than double the number of shots on goal, and a roughly five-to-six percent better clip of shots on target this year. 

Shooting stats alone don't necessarily mean that Maryland is a better team, but it certainly confirms that Maryland has been far improved in terms of its number of opportunities. Offense can't exist without opportunities. So it's all a bit of baby steps toward a better team. 