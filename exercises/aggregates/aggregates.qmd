...
```
**Answer** This is a list of 735 NBA players ordered by how many minutes they played. It includes several other bits of information including age, team, games played, rate of assists/blocks/steals and other analytical stats for each player. 
...
**Answer** This shows how many players there are at each position in the data set. I added a line of code to reorder it in descending order so that we can see there are a lot more shooting guards than any other position in the NBA, leading the way with 191 players. The next closest is small forwards with 139.
...
**Answer** Well, I got a step ahead in the last one. So same answer applies, there are way more shooting guards. We know the total number isn't exactly right because of repeated players, but based on the numbers we do see, there are 52 more shooting guards than the next closest position (about 7% more based on the flawed total).
...
I'd chase a story here with a bit of a question headline, something like: "Why does the NBA have so many shooting guards?" or "How an overpopulation of shooting guard complicates how much each one can get playing time." 

I go that route because we know there are comfortably more shooting guards than any other position and shooting guards account for the second largest minutes average. But that comes with a large standard deviation. So are there too many shooting guards? Is the NBA's modern desire to shoot 3s causing teams to roster too many players at that position, which thus create a massive minutes dispersion because, in reality, only so many shooting guards can play and a team is going to lean on their best players. Are lesser players being dragged along on active NBA rosters just because of a fad at that position, even if those players have no hope of actually playing? 
