...
matches_2025 <- matches_2025 |> 
  mutate(
    net_sog = so_g - defensive_so_g,
    net_corners = corners - defensive_corners,
    net_assists = assists - defensive_assists,
  )
```

```{r}
matrix <- matches_2025 |> select(goal_dif, net_sog, net_corners, net_assists)

cormatrix <- rcorr(as.matrix(matrix))

cormatrix$r
```

```{r}
wsoc_model <- lm(goal_dif ~ net_sog + net_corners + net_assists, data=matches_2025)
summary(wsoc_model)
```

This is a pretty good model overall. We have a high R squared value. But our correlation matrix shows a very high, maybe too high, correlation between net assists and goal differential. That may, in part, be cannibalizing each other because every assist is because of a goal, though not every goal has an assist. But we do have small residuals and a low p value so, as a whole, this seems usable despite the caveat. I picked these values because we knew from previous work that shots on goal matter. I tried to find other stats that weren't super on top of each other, but still has some relationship to scoring more goals. 


```{r}
matches_2025 <- matches_2025 |> mutate(predicted = predict(wsoc_model), residuals = residuals(wsoc_model))
```

```{r}
matches_2025 |> arrange(desc(residuals)) |> select(team, opponent, goal_dif, predicted, residuals) |> head(10)
```


```{r}
matches_2025 |> arrange(residuals) |> select(team, opponent, goal_dif, predicted, residuals) |> head(10)
```

Similar to our analysis of college basketball data, most of these large residuals/goal differentials are in games involving small/no name teams. And that generally makes sense because there will be a larger gap in talent. We do see more well known schools in this data frame than we saw in the college basketball data, and that may in part be due to the stretch of the season we are in now with still the bulk of the games played being non-conference games, and thus we may not have a ton of small school vs small school games yet. 


```{r}
matches_2025 |> filter(team == "Maryland Terrapins, Terps") |> select(team, opponent, goal_dif, predicted, residuals) |> arrange(desc(residuals))
```

Maryland's best performance relative to our model was its 2-1 win over Purdue. Here, the model predicted Maryland would lose by 1, but Maryland won by 1. So that's a two goal swing. The story here is simply how? What DID Maryland do that allowed it to overcome the model, and significantly outperform it? What can we see in our data that shows an element of chance in Maryland's games. Maryland has frequently performed pretty close to our model's expected outputs, so what is it about the outliers performances that we can take away?


If I were to pursue this further, I'd like to use more advanced stats to work toward a more efficient and predictive model. Obviously that is with data that isn't included in this data set, but I am fascinated by our conversations in class and in readings about some of the other metrics that could exist like AI software tracking what actually is a good shot or using the value of a player's possession of the ball into a model like this. I think some of those more advanced stats do more work to avoid having metrics in our model that speak for each other, as can be hard to avoid some of these data sets. 
...
teamzscore |> arrange(desc(TotalZscore)) |> select(team, TotalZscore) 
```
